...
def test_touch_home_macro_applies_thermal_delta_with_coeff(
    ...
    macro = TouchHomeMacro(
        ...
        travel_speed=50,
        random_radius=0,
        thermal_expansion_coefficient=coeff,
    )
    probe.perform_probe = mocker.Mock(return_value=trigger)
    toolhead.get_position = mocker.Mock(return_value=Position(0, 0, 2))
...
def test_touch_home_macro_uses_touch_config_thermal_coefficient(
    mocker: MockerFixture,
    probe: Probe,
    toolhead: Toolhead,
) -> None:
    params = MockParams()
    params.params["PRINT_TEMP"] = "140"

    trigger = 0.1
    coeff = 0.0004
    measured_touch_temp = 40.0
    expected = 2 - (trigger + coeff * (140 - measured_touch_temp))

    class ModelWithoutThermalCoefficient:
        name = "default"
        z_offset = 0.0

    probe.get_model = mocker.Mock(return_value=ModelWithoutThermalCoefficient())
    ...
    probe.perform_probe = mocker.Mock(return_value=trigger)
    toolhead.get_position = mocker.Mock(return_value=Position(0, 0, 2))
    toolhead.get_extruder_temperature = mocker.Mock(return_value=mocker.Mock(current=measured_touch_temp, target=40.0))
    set_z_position_spy = mocker.spy(toolhead, "set_z_position")
